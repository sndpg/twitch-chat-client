...
<2> invoke the `onBigCheer` as part of a tmiClient's onMessage actions. `_this_` corresponds to the function receiver `TmiSession`, and `_it_` is the incoming `TmiMessage` (lambda parameter).
