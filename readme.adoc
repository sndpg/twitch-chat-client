...
The following examples sends two messages to a channel through the sink and the simply prints these messages out to the console (they will be consumed as messages coming in from the channel which has been used by the sink to send the messages to):
